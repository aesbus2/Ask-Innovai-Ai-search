...
- Partner: {metadata_summary.get('partner', [])}
- Site: {metadata_summary.get('site', [])}
- LOB: {metadata_summary.get('lob', [])}
...
# Your existing @chat_router.post("/chat") stays unchanged

# ADD this new endpoint after your existing one:
@chat_router.post("/chat_enhanced")
async def chat_enhanced(request: Request):
    ...
    try:
        ...
        analysis_type = detect_analysis_type(req.message)
        logger.info(f"Enhanced chat - Analysis type: {analysis_type}")
        
        # Use your existing search function
        context, sources = build_search_context(req.message, req.filters, max_results=CHAT_MAX_RESULTS)

        if not context or len(sources) < 3:
            return JSONResponse(content={
                "reply": "Insufficient data for analysis. Please check filters or search terms.",
                "analysis_type": analysis_type,
                "sources": []
            })
        
        # Apply analysis-specific processing
        if analysis_type == "report":
            context = clean_context_for_reporting(context, sources)
            system_prompt = "Generate structured business report with statistics and tables."
        elif analysis_type == "pattern_search":
            context = enhance_context_for_patterns(context, sources)
            system_prompt = "Analyze transcripts for specific patterns and examples."
        elif analysis_type == "hybrid":
            context = build_hybrid_context(context, sources)
            system_prompt = "Generate structured report with supporting examples."
        else:
            system_prompt = "Provide business insights focusing on metadata analysis."
        
        system_message = f"{system_prompt}\n\nCONTEXT:\n{context}"
        
        # Use your existing Llama API call logic here
        # [Copy your existing API call code from relay_chat_rag]
        
        return JSONResponse(content={
            "reply": f"Enhanced response for {analysis_type} analysis",
            "analysis_type": analysis_type,
            "sources": sources[:10]
        })
        ...
        return JSONResponse(content={"reply": "Analysis failed", "error": str(e)})
