// Fixed chat.js - COMPLETE VERSION with working search functionality and fixed scroll behavior
...
function selectChat(session) {
    ...
    isSearchMode = false;
    renderSidebar();
    renderMessages(); // FIXED: Call without parameter, default will be used
    updateStarButton();
...
function addMessage(role, text, rawData) {
    ...
    renderMessages(role); // Pass the role to handle scroll behavior
}
...
// FIXED: Updated renderMessages function with proper parameter handling and null checks
function renderMessages(lastMessageRole = null) {
    if (isSearchMode) return; // Don't render messages in search mode

    const messagesDiv = document.getElementById('messages');
    
    // FIXED: Add null check for messagesDiv
    if (!messagesDiv) {
    ...
    const wasScrolledToBottom = messagesDiv.scrollHeight - messagesDiv.scrollTop <= messagesDiv.clientHeight + 50;
    ...
    // FIXED: Call with proper parameters and null checks
...
function handleScrollAfterRender(lastMessageRole, wasScrolledToBottom, messagesDiv) {
    ...
    if (!messagesDiv) {
        console.warn('handleScrollAfterRender: messagesDiv is null');
        return;
    }
...
function initializeApp() {
    ...
    renderSidebar();
    renderMessages(); // FIXED: Call without parameter
    updateStarButton();
